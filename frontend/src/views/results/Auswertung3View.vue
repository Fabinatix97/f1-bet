...
const voteCounts = computed(() => {
  ...
  props.betData.forEach((bet) => {
    const top3Drivers = bet.sideBets.top3Drivers
    if (top3Drivers && top3Drivers.length > 0) {
      top3Drivers.forEach((driverId) => {
  ...
  drivers.forEach((driver) => {
    ...
    if (count > 0) {
      const users = props.betData
        .filter((bet) => bet.sideBets.top3Drivers.includes(driver.id))
...
<template>
  <div>
    <h2>Welche beiden anderen Fahrer schaffen es in die Top 3?</h2>
    <div class="h-96 mt-4">
